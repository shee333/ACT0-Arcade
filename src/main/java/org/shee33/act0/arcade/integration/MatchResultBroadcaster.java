...
/**
 * 将街机赛后结果异步上报给 AstrBot QQ 群播报插件。
 *
 * <p>默认本机 webhook：{@code http://127.0.0.1:18710/act0/match-result}。
 * 可用 JVM 参数或环境变量覆盖：
 * <ul>
 *   <li>{@code -Dact0.qqbroadcast.enabled=false}</li>
 *   <li>{@code -Dact0.qqbroadcast.url=http://host:18710/act0/match-result}</li>
 *   <li>{@code -Dact0.qqbroadcast.token=...}</li>
 *   <li>{@code ACT0_QQ_BROADCAST_ENABLED=false}</li>
 *   <li>{@code ACT0_QQ_BROADCAST_URL=http://host:18710/act0/match-result}</li>
 *   <li>{@code ACT0_QQ_BROADCAST_TOKEN=...}</li>
 * </ul>
 */
public final class MatchResultBroadcaster {
    ...
    public static void sendArcadeResult(String matchId,
                                        String matchType,
                                        int durationSeconds,
                                        boolean draw,
        ...
        String eventId = "act0_arcade:" + matchId + ":" + durationSeconds;
        String json = "{"
                + field("event_id", eventId) + ","
                + field("source", "act0_arcade") + ","
                + field("match_id", matchId) + ","
                + field("match_type", matchType) + ","
                + numberField("duration_seconds", durationSeconds) + ","
                + booleanField("draw", draw) + ","
            ...
            CLIENT.sendAsync(builder.build(), HttpResponse.BodyHandlers.discarding())
                    .thenAccept(resp -> {
                        if (resp.statusCode() < 200 || resp.statusCode() >= 300) {
                            LOGGER.warn("[ACT0/Arcade] QQ group broadcast webhook returned HTTP {}", resp.statusCode());
                        }
                    })
                    .exceptionally(ex -> {
                        LOGGER.debug("[ACT0/Arcade] QQ group broadcast webhook failed: {}", ex.toString());
                        return null;
                    });
        } catch (Exception e) {
            LOGGER.debug("[ACT0/Arcade] failed to build QQ group broadcast webhook request: {}", e.toString());
        }
    }
